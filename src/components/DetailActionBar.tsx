...
interface DetailActionBarProps {
  ...
  onPrimary: () => void;
  primaryOnRight?: boolean;
  primaryActiveClassName?: string;
  primaryInactiveClassName?: string;
...
export default function DetailActionBar({
  ...
  primaryIcon,
  onPrimary,
  primaryOnRight = false,
  primaryActiveClassName = 'border border-accent bg-accent text-dark hover:brightness-105',
  primaryInactiveClassName = 'bg-accent text-dark hover:brightness-105',
  secondaryLabel,
  secondaryIcon,
  onSecondary,
  ...
  const primaryButton = (
    <button
      ...
      className={`flex flex-1 items-center justify-center gap-2 rounded-full px-4 py-3 text-sm font-black transition ${
        primaryActive ? primaryActiveClassName : primaryInactiveClassName
      }`}
    >
      {primaryIcon}
  ...
  return (
    <div className="fixed inset-x-0 bottom-5 z-40 mx-auto w-[calc(100%-2rem)] max-w-[24rem] rounded-full border border-white/10 bg-black/88 px-3 py-3 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-xl">
      <div className="flex items-center gap-3">
        {primaryOnRight ? secondaryButton : primaryButton}
        {primaryOnRight ? primaryButton : secondaryButton}
      </div>
    </div>
  );
}
